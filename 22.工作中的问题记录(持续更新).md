...
### 9.使用 **`websocketd + tail`** 来实现日志聚合查看

```
优点：非常轻量、无需复杂配置、无需数据库、可浏览器访问，
缺点：仅可以查看实时日志
```
...
```
        [Server1]    [Server2]    [Server3]
         tail -F       tail -F     tail -F
           │             │           │
       websocketd    websocketd   websocketd
           │             │           │
        ws://:6688    ws://:6688  ws://:6688
           └─────────────┬───────────┘
                         ▼
                浏览器聚合页面 (HTML+JS)
```

#### 每台服务器配置（WebSocket + tail）

安装 `websocketd`

```bash
[root@localhost ~]# curl -LO https://github.com/joewalnes/websocketd/releases/download/v0.4.1/websocketd-0.4.1-linux_amd64.zip
[root@localhost ~]# unzip websocketd-0.4.1-linux_amd64.zip
[root@localhost ~]# mv websocketd /usr/local/bin
[root@localhost ~]# chmod +x /usr/local/bin/websocketd
```

#### 启动 websocketd 服务监听日志

```bash
[root@localhost ~]# nohup websocketd --port=6688 --address=0.0.0.0 tail -F /opt/jar/logs/xxx.log &
```

#### 聚合前端 HTML 页面（部署在任意服务器）

创建一个网页 `log-aggregate.html`：
...
<head>
  ...
  <title>三列实时日志监控系统</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
  <style>
    * { margin:0; padding:0; box-sizing:border-box; font-family:'Segoe UI','Microsoft YaHei',sans-serif;}
    body { background:linear-gradient(135deg,#0f1a2d,#0c1117);color:#e0e0e0;min-height:100vh;overflow:hidden;}
    .container { display:flex;height:100vh;padding:20px;gap:20px;}
    .header { position:absolute;top:20px;left:50%;transform:translateX(-50%);text-align:center;z-index:10;}
    h1 { font-size:2.2rem;background:linear-gradient(90deg,#4dc9ff,#6e8efb);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:8px;text-shadow:0 2px 10px rgba(0,0,0,0.3);}    .subtitle { color:#8aa4d9;font-size:1rem;letter-spacing:1px;}
    .log-column { flex:1;background:rgba(15,25,45,0.7);border-radius:12px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 8px 32px rgba(0,20,80,0.4);border:1px solid rgba(64,156,255,0.15);backdrop-filter:blur(4px);transition:transform 0.3s ease;}
    .log-column:hover { transform:translateY(-5px);}
    .log-header { padding:15px 20px;background:rgba(25,45,75,0.8);border-bottom:1px solid rgba(64,156,255,0.2);display:flex;justify-content:space-between;align-items:center;}    
    .server-title { display:flex;align-items:center;gap:10px;font-size:1.2rem;font-weight:600;}
    .status-indicator { width:10px;height:10px;border-radius:50%;background:#4ade80;box-shadow:0 0 8px #4ade80;animation:pulse 1.5s infinite;}
    .server-info { font-size:0.85rem;color:#aac8ff;}
    .log-content { flex:1;padding:15px;overflow-y:auto;display:flex;flex-direction:column;}
    .log-line { padding:10px 12px;margin-bottom:8px;border-radius:6px;font-family:'Consolas','Courier New', monospace;font-size:0.92rem;line-height:1.5;white-space:pre-wrap;word-break:break-word;border-left:3px solid transparent;animation:fadeIn 0.25s ease-out;background:rgba(20,35,60,0.4);}
    .log-line:hover { background:rgba(30,50,85,0.6);}
    .error { color:#ff8a8a;border-left-color:#ff4c4c;background:rgba(120,30,40,0.3)!important;}
    .warn { color:#ffd166;border-left-color:#ffaa00;background:rgba(120,90,30,0.3)!important;}
    .info { color:#8aff80;border-left-color:#00ff00;}
    .debug { color:#a6b5ff;border-left-color:#6e8efb;}
    .timestamp { display:none; } /* 隐藏时间戳 */
    .log-content::-webkit-scrollbar { width:6px; }
    .log-content::-webkit-scrollbar-track { background:rgba(10,20,40,0.3); }
    .log-content::-webkit-scrollbar-thumb { background:#4a6cfb;border-radius:3px; }
    .log-content::-webkit-scrollbar-thumb:hover { background:#6e8efb; }
    @keyframes fadeIn { from { opacity:0;transform:translateY(5px);} to { opacity:1;transform:translateY(0);} }
    @keyframes pulse { 0% { opacity:0.6; } 50% { opacity:1; } 100% { opacity:0.6; } }
    .controls { position:absolute;bottom:25px;left:50%;transform:translateX(-50%);display:flex;gap:15px;z-index:10; }
    .btn { padding:10px 20px;background:rgba(30,60,120,0.6);color:#aac8ff;border:1px solid rgba(64,156,255,0.3);border-radius:8px;font-size:0.9rem;cursor:pointer;transition:all 0.3s;display:flex;align-items:center;gap:8px;}
    .btn:hover { background:rgba(40,80,160,0.7);transform:translateY(-3px);box-shadow:0 4px 15px rgba(64,156,255,0.3);}
    @media (max-width:900px) { .container { flex-direction:column;height:auto;min-height:100vh;} .log-column { min-height:33vh; } }
  </style>
</head>
<body>
  <div class="header">
    <h1><i class="fas fa-server"></i> 实时日志监控系统</h1>
    <p class="subtitle">三列并行展示 | 自动滚动粘底</p>
  </div>
  ...
  <div class="container">
    <div class="log-column">
      <div class="log-header">
        ...
        <div class="server-info">ws://10.99.16.16:6688</div>
      </div>
      <div class="log-content" id="Server-16-log"></div>
    </div>
    <div class="log-column">
      <div class="log-header">
        <div class="server-title">
        ...
        <div class="server-info">ws://10.99.16.17:6688</div>
      </div>
      <div class="log-content" id="Server-17-log"></div>
    </div>
    <div class="log-column">
      <div class="log-header">
        <div class="server-title">
        ...
        <div class="server-info">ws://10.99.16.18:6688</div>
      </div>
      <div class="log-content" id="Server-18-log"></div>
    </div>
  </div>

  <div class="controls">
    <button class="btn" id="pauseBtn"><i class="fas fa-pause"></i> 暂停/继续</button>
    <button class="btn" id="clearBtn"><i class="fas fa-trash-alt"></i> 清除日志</button>
    <button class="btn" id="scrollLockBtn"><i class="fas fa-lock"></i> 滚动锁定</button>
  </div>
  ...
  <script>
    ...
    function addLogEntry(logArea, text, level = null, serverColor = "#8aff80") {
      if (!level) {
        level = text.includes("ERROR") ? "error"
              : text.includes("WARN")  ? "warn"
              : text.includes("DEBUG") ? "debug"
              : "info";
      }
      const logLine = document.createElement("div");
      logLine.className = `log-line ${level}`;
      logLine.innerHTML = `${text}`;  // 删除时间戳展示
      logLine.style.borderLeftColor = serverColor;
      logArea.appendChild(logLine);
      logCount++;
      document.querySelector('.subtitle').textContent = `三列并行展示 | 日志总数: ${logCount}`;
      if (autoScrollEnabled) logArea.scrollTop = logArea.scrollHeight;
    }
    ...
    function initConnections() {
      servers.forEach(server => {
        try {
          ...
          ws.onopen = () => addLogEntry(server.logArea, "[INFO] 连接已建立", "info", server.color);
          ws.onmessage = e => { if (!isPaused) addLogEntry(server.logArea, e.data, null, server.color); };
          ws.onerror = () => addLogEntry(server.logArea, "[ERROR] WebSocket连接错误", "error", server.color);
          ws.onclose = () => addLogEntry(server.logArea, "[WARN] 连接已关闭", "warn", server.color);
          connections.push(ws);
        } catch (error) {
          addLogEntry(server.logArea, `[ERROR] 连接失败: ${error.message}`, "error", server.color);
        }
      });
    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
      servers = [
        { name: "Server‑16", url: "ws://10.99.16.16:6688", logArea: document.getElementById("Server-16-log"), color: "#8aff80" },
        { name: "Server‑17", url: "ws://10.99.16.17:6688", logArea: document.getElementById("Server-17-log"), color: "#a6b5ff" },
        { name: "Server‑18", url: "ws://10.99.16.18:6688", logArea: document.getElementById("Server-18-log"), color: "#ffd166" }
      ];
      ...
      document.getElementById("pauseBtn").addEventListener("click", () => {
        ...
        const color = isPaused ? "#ffd166" : "#4ade80";
        document.querySelectorAll('.status-indicator').forEach(i => {
          i.style.background = color;
          i.style.boxShadow = `0 0 8px ${color}`;
        });
      });

      document.getElementById("clearBtn").addEventListener("click", () => {
        document.querySelectorAll('.log-content').forEach(area => area.innerHTML = '');
        logCount = 0;
        document.querySelector('.subtitle').textContent = "三列并行展示 | 日志已清除";
      });

      document.getElementById("scrollLockBtn").addEventListener("click", () => {
        autoScrollEnabled = !autoScrollEnabled;
        const icon = document.querySelector("#scrollLockBtn i");
        icon.className = autoScrollEnabled ? "fas fa-lock" : "fas fa-lock-open";
      });

      document.querySelectorAll('.log-content').forEach(area => {
...
```bash
[root@localhost ~]# firewall-cmd --permanent --add-port=6688/tcp
[root@localhost ~]# firewall-cmd --reload
```

#### (PS:使用 systemd 守护 websocketd

文件：`/etc/systemd/system/websocketd-log.service`

```bash
Description=websocketd Log Stream
After=network.target

[Service]
ExecStart=/usr/local/bin/websocketd --port=6688 --address=0.0.0.0 tail -F /opt/jar/logs/xxx.log
Restart=always
User=root
...
[root@localhost ~]# systemctl enable websocketd-log --now
```

## )

### 10.minio使用命令行更改某个桶的权限

使用 MinIO 的命令行工具 `mc`（**MinIO Client**）来更改桶（bucket）的访问权限

#### 1. 安装 `mc` 工具

```bash
[root@00bb9448eaa2 /]# curl -O https://dl.min.io/client/mc/release/linux-amd64/mc
[root@00bb9448eaa2 /]# chmod +x mc
[root@00bb9448eaa2 /]# mv mc /usr/local/bin/
```

#### 2. 配置 MinIO 的别名连接

```bash
[root@00bb9448eaa2 /]# mc alias set local http://<minio-host>:<port> <MINIO_ACCESS_KEY> <MINIO_SECRET_KEY>
```

也可以使用用户名密码登陆

```bash
[root@00bb9448eaa2 /]# mc alias set local http://<minio-host>:<port> <MINIO_ROOT_USER> <MINIO_ROOT_PASSWORD>
```

#### 3. 设置桶权限为 public

```bash
[root@00bb9448eaa2 /]# mc anonymous set public  local/<桶名>
```
...
| 策略       | 命令                                     | 含义                       |
| ---------- | ---------------------------------------- | -------------------------- |
| `none`     | `mc anonymous set none local/<桶名>`     | 私有，只有授权用户可访问   |
| `download` | `mc anonymous set download local/<桶名>` | 公开只读（常用于静态文件） |
| `public`   | `mc anonymous set public local/<桶名>`   | 全部可读写（慎用）         |

#### 4. 查看策略

```bash
# 获取存储桶的匿名访问配置（文本格式）
[root@00bb9448eaa2 /]# mc anonymous get local/sdcit-bsfwdt
Access permission for `local/sdcit-bsfwdt` is `public`

# 获取存储桶的匿名访问配置（JSON格式）
[root@00bb9448eaa2 /]# mc anonymous get-json local/sdcit-bsfwdt
{
 "Statement": [
  {
   "Action": [
...
要删除名为 `bucket` 的桶，**先确保桶是空的**，否则 MinIO 不允许删除。

##### 5.1. 清空桶内所有对象：

```bash
[root@00bb9448eaa2 /]# mc rm --recursive --force local/bucket
```
...
```bash
[root@00bb9448eaa2 /]# mc rb local/bucket
```
